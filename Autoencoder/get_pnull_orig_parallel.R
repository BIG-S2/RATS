...
dist.array <- function(X, Y){
  ...
  for (i in 1:d){
    if (i %% 10 == 0){
      print(paste0("Number of dimensions finished for distance array: ",i))
    }
...
### function to calculate all statistics for a permuted sample with input a sequence of gamma
lgamma.cal.ed.seq = function(mat, nx, ny, gamma_seq){
  val_ed_mod = energymod.mat.seq(mat, nx, ny, gamma_seq)
  return(val_ed_mod)
}

lgamma.cal.bd.seq = function(mat, nx, ny, gamma_seq){
  val_bd_mod = bdmod.mat.seq(mat, nx, ny, gamma_seq)
  return(val_bd_mod)
}
...
lgamma.getnull = function(X, Y, perm){
  nx = dim(X)[1]
  ny = dim(Y)[1]
  dat = rbind(X, Y)
  dist = dist.array(X,Y)
...
lgamma.getnull.parallel = function(X, Y, dist, dist_ed, gamma_seq){
  nx = dim(X)[1]
  ny = dim(Y)[1]
  # dat = rbind(X, Y)
  ...
  value_perm_edmod = lgamma.cal.ed.seq(dist[sample, sample, ], nx, ny, gamma_seq)
  print("EDmod finished")
  value_perm_bdmod = lgamma.cal.bd.seq(dist[sample, sample, ], nx, ny, gamma_seq)
  print("BDmod finished")
  value_perm = c(value_perm_edmod, value_perm_bdmod)
