...
if (!file.exists(paste0("./r", ri, "/n", power_rep_n, "/X_", power_rep_n, ".RData"))){
  cov_mat = ar1_cor(size=dim, rho=0.6)
  X = mvrnorm(n, c(rep(mu_X, signal_dim), rep(0, dim-signal_dim)), cov_mat)
  Y = mvrnorm(m, rep(0, dim), cov_mat)
...
pvec = c(mu_X, pval_mmd, pval_ed, pval_ed1, pval_ed2, pval_ed3, pval_ed4, pval_ed5, pval_ed6, pval_edmax,
         pval_bd, pval_bd1, pval_bd2, pval_bd3, pval_bd4, pval_bd5, pval_bd6, pval_bdmax,
         minped_pval, minpbd_pval, minp_pval)
names(pvec) = c("mu", "MMD", "ED", "ED1", "ED2", "ED3", "ED4", "ED5", "ED6", "EDMAX",
